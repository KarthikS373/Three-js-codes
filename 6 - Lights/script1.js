...
const Disptexture = textureLoader.load("./earthDisp.png");
...
const sphere = new THREE.Mesh(
  new THREE.SphereBufferGeometry(2, 16, 16, 0, Math.PI * 2),
  material
);
...
const directionalLight = new THREE.DirectionalLight(0xffffff, 0.8); // THREE.DirectionalLight(color, intensity)
...
// const hemisphereLight = new THREE.HemisphereLight(0xff0000, 0x0000ff, 0.2); // THREE.HemisphereLight(color1, color2,intensity)
...
spotLight.target.position.x = -5; //note: spotLight.target.position
spotLight.target.position.y = -1; //note: spotLight.target.position
scene.add(spotLight);
